...
class OpenApiV3LibraryBundleTest {
    ...
    fun should_success_bundle() {
        wireMockRule.stubFor(
            WireMock.get(WireMock.urlEqualTo("/domain.yaml"))
                .willReturn(
                    WireMock.aResponse().withBody(IOUtils.toString(this.javaClass.getResource("stubs/Domain.yaml"), Charsets.UTF_8))
                )
        )
    ...
    fun should_get_errors() {
        ...
        Assert.assertThat(
            conflictingTypeNames["/components/schemas/Number"]!!.toList().get(0).toASCIIString(),
            endsWith("library-conflicts/domain-phone.yaml#")
        )
        Assert.assertThat(
            conflictingTypeNames["/components/schemas/Number"]!!.toList().get(1).toASCIIString(),
            endsWith("library-conflicts/domain-card.yaml#")
        )

        Assert.assertThat(
            conflictingTypeNames["/components/schemas/CurrencyCode"]!!.toList()[0].toASCIIString(),
            endsWith("library-conflicts/domain-monetary.yaml#")
        )
        Assert.assertThat(
            conflictingTypeNames["/components/schemas/CurrencyCode"]!!.toList()[1].toASCIIString(),
            IsEqual.equalTo("http://localhost:8888/domain.yaml#")
        )

        Assert.assertThat(
            conflictingTypeNames["/components/schemas/Expires"]!!.toList().get(0).toASCIIString(),
            endsWith("library-conflicts/domain-card.yaml#")
        )
        Assert.assertThat(
            conflictingTypeNames["/components/schemas/Expires"]!!.toList().get(1).toASCIIString(),
